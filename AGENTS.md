...
- Never commit secrets (HF tokens, `.env`, kubeconfigs, CA certs, workshop API URLs or tokens, real `questshift.llm.api-key`). Cluster secret `questshift-hf` via `oc create secret` only. `./install.sh` requires an existing `oc login`. Details: [WORKFLOWS.md](https://github.com/NA-FSI-Services/questshift/blob/main/docs/WORKFLOWS.md) (local `/Users/dtorresf/Documents/GitHub/na-fsi-services/questshift/questshift/docs/WORKFLOWS.md`).
