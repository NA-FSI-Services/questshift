...
| `golem` | 97 | `tile_0097` | Shell golem on the Broken Shell door until that room is solved |
...
Chest pickup opens a local dialog on the canvas (label + `text`, IBM Plex Mono). Esc / E / click the well closes it. Only the opener’s `partyMembers[].foundClues` hides that chest; teammates still see it.
