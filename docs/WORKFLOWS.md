...
Phase 3 (live GM narration): copy `questshift-engine/application-local.properties.example` to gitignored `application-local.properties`. Set `%dev.questshift.llm.enabled=true` plus `base-url`, `model`, and `api-key` for an OpenAI-compatible `/v1` server (local vLLM or a workshop MaaS chat-completions gateway). The overlay model id may be the gateway’s short Granite name; committed defaults stay **vLLM** + `ibm-granite/granite-3.2-8b-instruct` and `questshift.llm.api-key=none`. `%test` stays YAML-only. Cluster ConfigMap is the workshop runtime. Do not commit the overlay, workshop hostnames, or tokens. `LLMService` sends the player submission and the first YAML `accepted_examples` so Granite can give Game Master-tone feedback; YAML still scores. `LLMServiceTest` proves GM JSON keeps the YAML regex, the miss prompt carries the submission plus intended example, and HTTP errors fall back to YAML without calling a real endpoint.
...
3. Start a **new** session (`POST /api/sessions`) whenever you need another hour; live parties sit side by side. Join an existing hour with `GET /api/sessions/{joinCode}` then `POST /api/sessions/{id}/party`.
...
UI: a fresh tab shows the **quest lobby** (campaign card, character, alias, Start / Join). **import.yaml** on the lobby (and later on Panel B) posts the file to `POST /api/sessions/import` (works with or without a live session; an imported `active` hour sits beside others). After Start, dual-panel play: Panel B **export.yaml** downloads `questshift-{id}.yaml`. Topbar **Party {joinCode}** holds Copy code, Switch party, new party, Abandon, and Delete (type the join code). Switch / new party return to the lobby.
