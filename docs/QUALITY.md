...
- Phase 3 CI: `LLMServiceTest` enabled-narrate cases keep YAML `expectedCommandPattern`, send the player submission plus first `accepted_examples` on a miss, and fall back on HTTP errors. Do not call a live vLLM in `./mvnw test`.
