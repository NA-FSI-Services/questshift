...
7. Never commit secrets to GitHub. Hugging Face token is cluster secret `questshift-hf` (`./install.sh` or `oc create secret`); git may reference the name only. Workshop API URLs, tokens, kubeconfigs, and CA certs stay in a local `oc` session or gitignored `.env`. See [WORKFLOWS.md](https://github.com/NA-FSI-Services/questshift/blob/main/docs/WORKFLOWS.md) (local `/Users/dtorresf/Documents/GitHub/na-fsi-services/questshift/questshift/docs/WORKFLOWS.md`).
